...
This is still external data, so a pointer from Synapse to the CSV file located on the Azure storage account.
A key use case for data virtualization with the PolyBase is to allow the data to stay in its original location and format. 
You can virtualize the external data through the SQL Server instance, so that it can be queried in place like 
any other table in SQL Server. 
This process minimizes the need for ETL processes for data movement. 
Resource: https://docs.microsoft.com/en-us/sql/relational-databases/polybase/polybase-guide?view=sql-server-ver15
*/
...
-- Now let's create the external tables
-- We define column names and data types, and binding them to the location and format of the Azure Azure storage account files
-- The location is the folder under the root directory of the Azure storage account
